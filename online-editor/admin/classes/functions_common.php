...
$andor = 'AND';
  	...
  	//$search_sql[]= " user_fullname like '%".trim($searchparam['keywords'])."%' ";
		...
		if($searchparam['order']){
		 ...
		 $query = "select * from programs where 1  $search_sql_query order by $orderby";

		 }

		 else{

		 $query = "select * from programs where 1   $search_sql_query order by created_date DESC";
...
//echo $query;
		...
		function get_program($start=0, $perpage=0, $searchparam){
...
if($searchparam['sortby']==''){
		...
		//$query = "select * from registration where product_type='".$website."' and isvendor='0' order by id desc"; // old without searching
		 ...
		 $query = "select * from programs where student_id='$_SESSION[userid]' $search_sql_query order by id $sortby";
